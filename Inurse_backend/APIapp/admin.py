...
class UserChangeForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ('email', 'password', 'groups', 'is_active', 'is_superuser')
...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ('username', 'email', 'dni', 'first_name', 'last_name', 'is_staff', 'groups')
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username', 'first_name', 'last_name', 'email', 'dni', 'groups', 'password1', 'password2'),
        }),
    )
    search_fields = ('email',)
    ordering = ('email',)
